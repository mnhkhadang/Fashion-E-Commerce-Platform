...
const STATUS_CONFIG = {
  PENDING:  { label: 'Đang chờ duyệt', color: 'text-yellow-600 bg-yellow-50 border-yellow-200', icon: '⏳' },
  APPROVED: { label: 'Đã được duyệt',  color: 'text-green-600 bg-green-50 border-green-200',   icon: '✅' },
  REJECTED: { label: 'Bị từ chối',     color: 'text-red-500 bg-red-50 border-red-200',         icon: '❌' },
}
...
export default function RegisterShopStatus() {
  ...
  useEffect(() => {
    api.get('/api/shop-registrations/my')
                  ...
                  <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-100">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        Gửi lúc: {new Date(reg.createdAt).toLocaleDateString('vi-VN', {
                          day: '2-digit', month: '2-digit', year: 'numeric',
                          hour: '2-digit', minute: '2-digit'
                        })}
                      </p>
                    </div>
                    <span className={`text-xs font-medium px-3 py-1.5 rounded-full border flex items-center gap-1.5 ${s.color}`}>
                      {s.icon} {s.label}
                    </span>
                  </div>

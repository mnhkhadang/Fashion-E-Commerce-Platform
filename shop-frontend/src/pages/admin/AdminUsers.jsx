...
export default function AdminUser() {
    ...
    const handleToggle = async (email) => {
        ...
        try {
            ...
            setUsers(prev => prev.map(u => u.email === email ? { ...u, enable: !u.enable } : u))
        } catch {
            alert('Có lỗi xảy ra')
        } finally {
            setProcessing(null)
    ...
    const handleAssignRole = async (email, role) => {
        ...
        try {
            ...
            setUsers(prev => prev.map(u => u.email === email ? { ...u, role: [...u.role, role] } : u))
        } catch (err) {
    ...
    const handleRemoveRole = async (email, role) => {
        ...
        try {
            ...
            setUsers(prev => prev.map(u => u.email === email ? { ...u, role: u.role.filter(r => r !== role) } : u))
        } catch (err) {
            alert(err.response?.data || 'Có lỗi xảy ra')
        } finally {
            setProcessing(null)
            ...
            <div className="flex-1 px-6 py-5">
                ...
                {loading ? (
                    ...
                    <div className="bg-white rounded-lg shadow-sm overflow-hidden">
                                                    ...
                                                    <tr className="border-b border-gray-50 hover:bg-gray-50 transition">
                                                        ...
                                                        <td className="px-5 py-3 w-1/4">
                                                            <div className="flex flex-wrap gap-1">
                                                                {user.role?.map(r => (
                                                                    <span key={r} className="text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded">
                                                                        {r.replace('ROLE_', '')}
                                                                    </span>
                                                                ))}
                                                            </div>
                                                        ...
                                                        <td className="px-5 py-3 w-1/4">
                                                            <span className={`text-xs px-2 py-0.5 rounded-full ${user.enable ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                                        ...
                                                        <tr className="bg-blue-50">
                                                            <td colSpan={4} className="px-5 py-3">
                                                                <p className="text-xs font-semibold text-gray-600 mb-2">
                                                                    Quản lý role cho: <span className="text-blue-600">{user.email}</span>
                                                                </p>
                        ...
                        {filtered.length === 0 && (
                            <div className="text-center py-12 text-gray-400 text-sm">Không tìm thấy người dùng nào</div>
                        )}
                    </div>
                )}
            </div>

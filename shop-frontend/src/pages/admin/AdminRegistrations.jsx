...
const STATUS_CONFIG = {
  ...
  REJECTED: { label: 'Từ chối',   color: 'text-red-500 bg-red-50 border-red-200' },
}
...
export default function AdminRegistrations() {
  ...
  useEffect(() => {
    api.get('/api/shop-registrations')
  ...
  const handleApprove = async (id) => {
    ...
    try {
      const res = await api.post(`/api/shop-registrations/${id}/approve`)
  ...
  const handleReject = async () => {
    ...
    try {
      const res = await api.post(`/api/shop-registrations/${rejectModal.id}/reject?reason=${encodeURIComponent(rejectReason)}`)
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 px-6 py-5">
        ...
        <div className="bg-white rounded-lg shadow-sm mb-4 flex">
          {[['PENDING', 'Chờ duyệt'], ['APPROVED', 'Đã duyệt'], ['REJECTED', 'Từ chối'], ['ALL', 'Tất cả']].map(([val, label]) => (
            <button key={val} onClick={() => setFilterStatus(val)}
              className={`px-5 py-3 text-sm font-medium border-b-2 transition cursor-pointer bg-white border-l-0 border-r-0 border-t-0 ${
                filterStatus === val ? 'border-orange-500 text-orange-500' : 'border-transparent text-gray-500 hover:text-orange-400'
              }`}>
              {label}
                ...
                <div key={reg.id} className="bg-white rounded-lg shadow-sm p-5">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {reg.description && <p className="text-xs text-gray-400 mt-1">{reg.description}</p>}
                      <p className="text-xs text-gray-300 mt-1.5">
                        Gửi: {new Date(reg.createdAt).toLocaleDateString('vi-VN', {
                          day: '2-digit', month: '2-digit', year: 'numeric',
                          hour: '2-digit', minute: '2-digit'
                        })}
                      </p>
                      {reg.rejectReason && <p className="text-xs text-red-400 mt-1">❌ Lý do: {reg.rejectReason}</p>}
                    </div>
                    {reg.status === 'PENDING' && (
                      <div className="flex gap-2 ml-4 shrink-0">
                        ...
                        <button onClick={() => { setRejectModal({ id: reg.id, shopName: reg.shopName }); setRejectReason('') }}
      ...
      {rejectModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
            ...
            <p className="text-xs text-gray-400 mb-3">Shop: <span className="font-medium text-gray-600">{rejectModal.shopName}</span></p>
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={handleReject} disabled={processing?.includes('_reject')}

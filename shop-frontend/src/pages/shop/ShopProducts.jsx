...
export default function ShopProducts() {
    ...
    const removeMedia = (idx) => {
        setForm(prev => ({
            ...prev,
            mediaList: prev.mediaList.filter((_, i) => i !== idx).map((m, i) => ({ ...m, sortOrder: i }))
        }))
    }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setCatSearch('')
            setEditingId(null)
            setSavedMessage(editingId? 'Cập nhật sản phẩm thành công' : 'Thêm sản phẩm thành công')
            setSaved(true)
        ...
        <div className='min-h-screen bg-gray-50 flex'>
            ...
            <div className='flex-1 overflow-auto'>
                <div className='max-w-7xl mx-auto px-6 py-6'>
                    ...
                    {saved && (
                        <div className='flex items-center gap-2 bg-green border border-green-200 text-green-600 text-sm px-4 py-3 rounded-xl mb-4'>
                             ✅ {savedMessage}
                        </div>
                    )}
                    ...
                    {showForm && (
                        <div className='bg-white rounded-xl shadow-sm p-5 mb-4'>
                            ...
                            <div className='grid grid-cols-1 gap-3'>
                                            ...
                                            <div className='absolute z-10 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto'>
                                                {filteredCategories.map(cat => (
                                                    <div
                                                        ...
                                                        className={`px-3 py-2 text-sm cursor-pointer hover:bg-orange-50 hover:text-orange-500 ${form.categoryId === cat.id ? 'bg-orange-50 text-orange-500 font-medium' : 'text-gray-700'}`}
                                                    >
                                                        {cat.name}
                                                    </div>
                                                ))}
                                ...
                                <div>
                                    ...
                                    {form.mediaList.length > 0 && (
                                        <p className='text-xs text-gray-400 mt-1'>{form.mediaList.length} ảnh đã chọn</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className='flex gap-2 mt-4'>
                                ...
                                <button onClick={() => { setShowForm(false); setEditingId(null); setCatSearch('') }}
                    ...
                    {filtered.length === 0 ? (
                        ...
                        <div className='space-y-3'>
                            {filtered.map(product => (
                                <div key={product.id} className={`bg-white rounded-xl shadow-sm p-4 flex items-center gap-4 ${!product.active ? 'opacity-60' : ''}`}>
                                    ...
                                    <div className='flex-1 min-w-0'>
                                        <div className='flex items-center gap-2'>
                                            ...
                                            {!product.active && <span className='text-[10px] bg-gray-100 text-gray-400 px-1.5 py-0.5 rounded shrink-0'>Đã ẩn</span>}
                                        </div>
                                        <p className='text-xs text-gray-400'>{product.categoryName}</p>
                                        <div className='flex items-center gap-3 mt-1'>
                                            <span className='text-sm font-bold text-orange-500'>{Number(product.price).toLocaleString('vi-VN')}₫</span>
                                    ...
                                    <div className='flex gap-2 shrink-0'>
                                        ...
                                        <button onClick={() => handleToggle(product.id)}
                                            className={`text-xs border px-3 py-1.5 rounded-lg transition cursor-pointer bg-white ${product.active ? 'border-gray-300 text-gray-500 hover:border-red-300 hover:text-red-400' : 'border-green-300 text-green-500 hover:bg-green-50'}`}>

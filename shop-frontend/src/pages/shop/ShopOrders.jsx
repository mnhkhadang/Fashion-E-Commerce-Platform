...
const STATUS_CONFIG = {
    PENDING:   { label: 'Chờ xác nhận', color: 'text-yellow-700 bg-yellow-50 border-yellow-200', next: 'CONFIRMED', nextLabel: 'Xác nhận đơn' },
    CONFIRMED: { label: 'Đã xác nhận',  color: 'text-blue-700 bg-blue-50 border-blue-200',      next: 'SHIPPING',  nextLabel: 'Giao hàng ngay' },
    SHIPPING:  { label: 'Đang giao',    color: 'text-orange-700 bg-orange-50 border-orange-200', next: 'DELIVERED', nextLabel: 'Đã giao hàng' },
    DELIVERED: { label: 'Đã giao',      color: 'text-green-700 bg-green-50 border-green-200',    next: null },
    CANCELLED: { label: 'Đã hủy',       color: 'text-red-600 bg-red-50 border-red-200',          next: null },
}
...
export default function ShopOrders() {
    ...
    const handleUpdateStatus = async (orderCode, newStatus) => {
        ...
        try {
            const res = await api.put(`/api/orders/${orderCode}/status?status=${newStatus}`)
    ...
    const filtered = orders.filter(order => {
        ...
        if (statusFilter && !statusFilter.includes(order.orderStatus)) return false
        return true
    })
                    ...
                    <div className='bg-white rounded-2xl shadow-sm mb-6 flex p-1 border border-gray-100 overflow-x-auto'>
                        {TABS.map(tab => (
                            <button key={tab} onClick={() => setActiveTab(tab)}
                                className={`flex-1 flex-shrink-0 px-6 py-4 text-base font-bold rounded-xl transition cursor-pointer border-0 ${
                                    activeTab === tab
                                        ? 'bg-orange-500 text-white shadow-md'
                                        : 'bg-transparent text-gray-500 hover:text-orange-500'
                                }`}>
                                {tab}
                                <span className={`ml-2 text-sm ${activeTab === tab ? 'text-orange-100' : 'text-gray-400'}`}>
                                    ({tab === 'Tất cả' ? orders.length : orders.filter(o => TAB_STATUS[tab]?.includes(o.orderStatus)).length})
                                </span>
                            </button>
                        ))}
                    </div>
                    ...
                    {filtered.length === 0 ? (
                        ...
                        <div className='space-y-4'>
                            {filtered.map(order => {
                                ...
                                return (
                                    <div key={order.orderCode} className='bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100 hover:shadow-md transition-shadow'>
                                        {/* Header */}
                                        <div className='px-6 py-5 border-b border-gray-50 flex items-center justify-between bg-white'>
                                            <div className='flex items-center gap-6'>
                                                ...
                                                <div>
                                                    <p className='text-xs font-bold text-gray-400 uppercase tracking-wider mb-1'>Ngày đặt hàng</p>
                                            ...
                                            <span className={`text-sm font-black px-4 py-1.5 rounded-full border-2 ${status.color}`}>
                                        ...
                                        <div className='px-6 py-5'>
                                            {order.items.slice(0, isExpanded ? order.items.length : 2).map((item, idx) => (
                                                <div key={idx} className='flex items-center gap-4 mb-4 last:mb-0'>
                                                    <div className='w-14 h-14 bg-gray-50 rounded-xl flex items-center justify-center text-gray-300 shrink-0 border border-gray-100 text-2xl'>📦</div>
                                                    <div className='flex-1 min-w-0'>
                                                        <p className='text-base font-bold text-gray-700 truncate'>{item.productName}</p>
                                                        <p className='text-sm font-bold text-gray-400'>Số lượng: <span className='text-gray-600'>{item.quantity}</span></p>
                                                    </div>
                                            ...
                                            {order.items.length > 2 && (
                                                <button onClick={() => setExpandedOrder(isExpanded ? null : order.orderCode)}
                                                    className='text-sm font-bold text-orange-500 mt-2 cursor-pointer bg-transparent border-0 p-0 hover:underline'>
                                                    {isExpanded ? '▲ Thu gọn danh sách' : `▼ Xem thêm ${order.items.length - 2} sản phẩm khác`}
                                                </button>
                                            )}
                                        </div>
                                        ...
                                        {/* Shipping info */}
                                        {isExpanded && (
                                            <div className='px-6 py-5 bg-orange-50 border-t border-orange-100 flex gap-8 transition-all'>
                                                <div className='flex-1'>
                                                    <p className='text-xs font-black text-orange-400 uppercase tracking-widest mb-2'>📍 Thông tin nhận hàng</p>
                                                    <p className='text-base font-black text-gray-800'>{order.shippingFullName} — {order.shippingPhone}</p>
                                                ...
                                                {order.note && (
                                                    <div className='flex-1 border-l border-orange-200 pl-8'>
                                                        <p className='text-xs font-black text-orange-400 uppercase tracking-widest mb-2'>💬 Ghi chú khách hàng</p>
                                        ...
                                        <div className='px-6 py-4 bg-gray-50 flex items-center justify-between border-t border-gray-100'>
                                            <p className='text-lg font-bold text-gray-700'>
                                                Tổng thanh toán: <span className='text-2xl font-black text-orange-500 ml-2'>{Number(order.totalPrice).toLocaleString('vi-VN')}₫</span>
                                            </p>
                                            <div className='flex gap-3'>
                                                ...
                                                {status.next && (
                                                    <button
                                                        onClick={() => handleUpdateStatus(order.orderCode, status.next)}
                                                        ...
                                                        {updating === order.orderCode ? 'Đang xử lý...' : status.nextLabel}
                                                    </button>
                                                )}
                                            </div>
                                        </div>

...
export default function ShopProfile() {
    ...
    const [form, setForm] = useState({ name: '', description: '', address: '', phone: '', avatar: '' })
    const [loading, setLoading] = useState(true)
    const [submitting, setSubmitting] = useState(false)
    const [saved, setSaved] = useState(false)

    useEffect(() => {
        ShopService.getMyShop().then(res => {
            setShop(res.data)
            setForm({
                name: res.data.name || '',
    ...
    const handleSubmit = async () => {
        if (!form.name) { alert('Tên shop không được để trống'); return }
        setSubmitting(true)
        try {
                ...
                <div className='max-w-7xl mx-auto px-8 py-8'>
                    ...
                    <div className='bg-white rounded-2xl shadow-md p-8 mb-8 border border-gray-100'>
                        ...
                        <div className='flex items-center gap-6 mb-8 pb-8 border-b border-gray-100'>
                            ...
                            <div>
                                ...
                                <span className={`text-xs px-3 py-1 rounded-full mt-2 inline-block font-black uppercase tracking-wider ${shop?.active ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-500'}`}>
                        ...
                        <div className='grid grid-cols-1 gap-6'>
                            ...
                            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                                <div>
                                    <label className='text-sm font-bold text-gray-600 mb-2 block'>Số điện thoại liên hệ</label>
                                    <input type="text" value={form.phone}
                                        onChange={e => setForm(p => ({ ...p, phone: e.target.value }))}
                                        className='w-full border border-gray-200 rounded-xl px-4 py-3 text-base outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition-all font-medium' />
                                </div>
                                <div>
                                    <label className='text-sm font-bold text-gray-600 mb-2 block'>Đường dẫn ảnh đại diện (URL)</label>
                            ...
                            <div>
                                <label className='text-sm font-bold text-gray-600 mb-2 block'>Giới thiệu về shop</label>
                                <textarea value={form.description}
                                    onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
                                    rows={4}
                                    placeholder="Chia sẻ câu chuyện hoặc phương châm bán hàng của bạn..."
                        ...
                        <div className='flex items-center gap-4 mt-8'>
                            ...
                            {saved && (
                                <div className='flex items-center gap-2 text-green-600 font-bold animate-bounce'>
                                    <span className='text-xl'>✓</span> Cập nhật thành công!
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                        <div className='bg-white rounded-2xl shadow-sm p-6 border border-gray-50'>
                            <p className='text-sm font-bold text-gray-400 mb-2 uppercase tracking-widest'>Ngày gia nhập hệ thống</p>
                            <p className='text-xl font-black text-gray-700'>
                                {shop?.createdAt ? new Date(shop.createdAt).toLocaleDateString('vi-VN', { day: '2-digit', month: 'long', year: 'numeric' }) : '—'}
                            </p>
                        </div>
                        <div className='bg-white rounded-2xl shadow-sm p-6 border border-gray-50'>
